...
class Repo:
    def save_snapshot(self, s): self.s = deepcopy(s)
    def load_latest(self, memory_id): return deepcopy(self.s)
    def save_incident(self, **kwargs): pass
    def recall_incidents(self, **kwargs): return [{"text":"stored"}]
    def tooling_evidence(self): return {"vector_index":"verification_incidents_embedding_idx","verified_memory_rows":1}
class Model:
    def embed(self, text): return [0.0] * 1024
    def explain(self, result): return "Explanation only."

def test_fresh_session_and_separate_applicability():
    repo = Repo(); service = EvidenceBoundService(repo, Model())
    saved = service.save_t0(); reopened = service.reopen_t1(saved["memory_id"])
...
def test_modified_history_fails_closed():
    repo = Repo(); service = EvidenceBoundService(repo, Model())
    saved = service.save_t0(); repo.s["evidence"][0]["value"] = "modified"
