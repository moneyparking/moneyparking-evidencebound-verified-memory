...
class CockroachRepository:
    ...
    def save_snapshot(self, snapshot: dict[str, Any]) -> None:
        ...
        query = sql.SQL(
            "INSERT INTO {} (memory_id, session_id, created_at, policy_version, proof_version, decision_state, evidence_hash, record_hash, previous_record_hash, snapshot) "
            "VALUES (%s::UUID, %s::UUID, %s::TIMESTAMPTZ, %s, %s, %s, %s, %s, %s, %s)"
    ...
    def load_latest(self, memory_id: str) -> dict[str, Any] | None:
        ...
        query = sql.SQL(
            "SELECT snapshot FROM {} WHERE memory_id = %s::UUID ORDER BY created_at DESC, id DESC LIMIT 1"
        ).format(sql.Identifier(MEMORY_TABLE))
        with self._connect() as conn, conn.cursor() as cur:
    ...
    def save_incident(self, *, memory_id: str, incident_type: str, text: str, embedding: list[float]) -> None:
        ...
        query = sql.SQL(
            "INSERT INTO {} (memory_id, incident_type, incident_text, embedding) VALUES (%s::UUID, %s, %s, %s::VECTOR)"
    ...
    def recall_incidents(self, *, memory_id: str, embedding: list[float], limit: int = 3) -> list[dict[str, Any]]:
        ...
        query = sql.SQL(
            "SELECT incident_type, incident_text, embedding <-> %s::VECTOR AS distance "
            "FROM {} WHERE memory_id = %s::UUID ORDER BY embedding <-> %s::VECTOR LIMIT %s"
        ).format(sql.Identifier(INCIDENT_TABLE))
        with self._connect() as conn, conn.cursor() as cur:
            cur.execute(query, (vector, memory_id, vector, limit))
            rows = cur.fetchall()
    ...
    def tooling_evidence(self) -> dict[str, Any]:
        ...
        index_query = sql.SQL(
            "SELECT index_name FROM [SHOW INDEXES FROM {}] WHERE index_name = %s LIMIT 1"
        ).format(sql.Identifier(INCIDENT_TABLE))
